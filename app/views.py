...
class ProdutoViewSet(
    ...
    *Descrição: 
        Faz uma query de todos os produtos e 
        faz uma filtragem baseada nos parametros baseados pela URL. 
        Parametros: nomeProduto,precoMaximo,precoMinimo, raio, banca, 
        feira,latitude e longitude do cliente. 
        Para retornar a distancia devem ser fornecido raio, latitude e longitude do cliente. 
        Exemplo: http://127.0.0.1:8000/produtos/?raio=14&latitudeCliente=50.1&longitudeCliente=50.1

    * Parametros
        request - objeto de HttpRequest

    * Valor retornado
        Retorna um JsonResponse com todos os produtos que atendiam as condicoes passadas pela 
        URL. Caso seja um metodo POST e o 
        objeto tenha sido criado, 
        retorna uma Response com status 201_CREATED junto com os dados do novo produto
...
"""
    ...
    def get(self, request):
        ...
        if latitudeCliente and longitudeCliente:
            ...
            produtos = Produto.objects.annotate(
                distancia=ExpressionWrapper(
                    ACos(
                        Cos(Radians(latitudeCliente))
                        * Cos(Radians(F("fornecedor__latitude")))
                        * Cos(
                            Radians(F("fornecedor__longitude"))
                            - Radians(longitudeCliente)
                        )
                        + Sin(Radians(latitudeCliente))
                        * Sin(Radians(F("fornecedor__latitude")))
                    )
        ...
        if fornecedorProduto:
            produtos = produtos.filter(
                Q(fornecedor__nome_do_negocio__icontains=fornecedorProduto)
            )
    ...
    ***************************************************************************/ 
    ...
    @extend_schema(
        description="É criado um novo produto no banco de dados a partir das informações        passadas, sendo elas, nome, descricao, preco e id da categoria. O usuario autenticado é um fornecedor e será associado ao produto."
    )
...
class CategoriaViewSet(
    ...
    ***************************************************************************/
    ...
    @extend_schema(
        description="Retorna todas as categorias registradas. Feita para utilizar no cadastro dos produtos"
    )
    def get(self, request):
...
class CompraViewSet(
    ...
    ***************************************************************************/
    ...
    @extend_schema(
        description="Retorna as compras do cliente logado. Não preencher o campo cliente, ele será puxado do usuario autenticado"
    )
    def get_cliente_compras(self, request):
        try:
            ...
            return Response(
                "Usuario logado não é cliente", status=status.HTTP_403_FORBIDDEN
            )
    ...
    ***************************************************************************/
    ...
    @extend_schema(
        description="Realiza uma nova compra no perfil do cliente logado. Não precisa preencher o campo cliente, ele será puxado do usuario autenticado"
    )
    def post(self, request):
        try:
            cliente = Cliente.objects.get(cliente_user=request.user)
    ...
    ***************************************************************************/
    ...
    @extend_schema(
        description="Retorna uma única compra a partir do ID passado na URL."
    )
    def get_specific(self, request, id):
        try:
    ...
    ***************************************************************************/
    ...
    @extend_schema(
        description="Deleta uma compra com base no ID passado na URL. Usuario deve estar autenticado"
    )
    def delete(self, request, id):
        try:
            compra = Compra.objects.get(pk=id)
...
class UsuarioViewSet(
    ...
    * Assertica de entrada
    ...
    @extend_schema(
        description="Cria um usuario o qual devera ser cliente ou fornecedor"
    )
    def post(self, request):
...
class FornecedorViewSet(
    ...
    * Assertica de entrada
    ...
    @extend_schema(
        description="É criado um novo objeto no banco de dados a partir das informações        passadas, sendo elas, nome_do_negocio, endereco, latitude, longitude, feira. Deve ter um usuario is_fornecedor logado para que ele seja associado ao novo fornecedor"
    )
    def post(self, request):
        data = request.data
    ...
    ***************************************************************************/ 
    ...
    @extend_schema(
        description="Retorna um unico fornecedor a partir de id passada na URL"
    )
    def get_specific(self, request, id):
        try:
...
class FeiraViewSet(
    ...
    ***************************************************************************/
    ...
    @extend_schema(
        description="Retorna todas as feiras cadastradas no sistema juntamente com seu ID. Feita para usar no cadastro de um Forncedor"
    )
    def get(self, request):
...
class ClienteViewSet(
    ...
    ***************************************************************************/
    ...
    @extend_schema(
        description="Cria um novo cliente no banco de dados com as informações passadas, incluindo nome, email e telefone. O usuário logado deve ser do tipo cliente para ser associado ao novo cliente"
    )
    def post(self, request):
        data = request.data
    ...
    ***************************************************************************/
    ...
    @extend_schema(
        description="Retorna um único cliente com base no ID passado na URL."
    )
    def get_specific(self, request, id):
        try:
    ...
    ***************************************************************************/
    ...
    @extend_schema(
        description="Atualiza o cliente com base no ID passado na URL. O usuario deve estar logado"
    )
    def put(self, request, id):
        try:
            cliente = Cliente.objects.get(pk=id)
    ...
    ***************************************************************************/
    ...
    @extend_schema(
        description="Deleta o cliente com base no ID passado na URL. O usuario deve estar logado"
    )
    def delete(self, request, id):
        try:
            cliente = Cliente.objects.get(pk=id)
...
class MensagemViewSet(
    ...
    ***************************************************************************/
    ...
    @extend_schema(
        description="Será feita uma query que retornará todas as mensagens entre dois usuarios. Retorna de maneira ordenada"
    )
    def get_msg_cliente_fornecedor(self, request, user2):
        ...
        mensagens = Mensagem.objects.filter(
            (Q(destinatario_id=user1) & Q(remetente_id=user2))
            | (Q(destinatario_id=user2) & Q(remetente_id=user1))
        ).order_by("data_envio")
    ...
    def get_diferentes_usuarios_chat(self,request):
        remetentes = (
            Mensagem.objects.filter(
                ~Q(remetente_id=request.user.id) & Q(destinatario_id=request.user.id)
            )
            .values("remetente_id", "remetente__username")
            .distinct()
        )
    ...
    ***************************************************************************/
    ...
    @extend_schema(
        description="É criada uma nova mensagem no banco de dados a partir das informações passadas, incluindo destinatario, conteudo e data_envio. O usuario devera estar autenticado para que ele seja o remetente da mensagem no banco de dados"
    )
    def post(self, request):
        # data = request.data
...
class AvaliacaoViewSet(
    ...
    ***************************************************************************/
    ...
    @extend_schema(
        description="retorna todas as avaliações de um produto do banco de dados"
    )
    ...
    ***************************************************************************/
    ...
    @extend_schema(
        description="É criada uma nova avaliação no banco de dados a partir das informações passadas, incluindo produto ,nota e comentario. É necessario estar autenticado para que a nova avaliação seja associada ao usuario"
    )
    def post(self, request):
        data = request.data
    ...
    ***************************************************************************/
    ...
    @extend_schema(
        description="Retorna uma única avaliação a partir do ID passado na URL."
    )
    def get_specific(self, request, id):
        try:
    ...
    ***************************************************************************/
    ...
    @extend_schema(
        description="Atualiza uma avaliação com base no ID passado na URL. O usuario deve estar logado"
    )
    def put(self, request, id):
        try:
            avaliacao = Avaliacao.objects.get(pk=id)
    ...
    ***************************************************************************/
    ...
    @extend_schema(
        description="Deleta uma avaliação com base no ID passado na URL. O usuario deve estar logado"
    )
    def delete(self, request, id):
        try:
            avaliacao = Avaliacao.objects.get(pk=id)
